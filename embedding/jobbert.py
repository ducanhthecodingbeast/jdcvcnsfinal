"""
JobBERT-v2 embeddings for JD job title to CV skills matching.

Scores similarity between JD job title and CV skills pairs using TechWolf/JobBERT-v2.
...
MODEL_CACHE_DIR = Path(__file__).parent.parent / "dataset" / "model" / "jobbert-v2"


def load_jobbert_model() -> SentenceTransformer:
    """
    Load JobBERT-v2 model for job title and skills matching.
    Uses local cache if available, otherwise downloads.
    """
    device = "cuda" if torch.cuda.is_available() else "cpu"
    logger.info(f"Loading JobBERT-v2 model on device: {device}")

    if MODEL_CACHE_DIR.exists():
        logger.info(f"Loading from local cache: {MODEL_CACHE_DIR}")
        model = SentenceTransformer(str(MODEL_CACHE_DIR), device=device)
    else:
        logger.info(f"Downloading model: {JOBBERT_MODEL_NAME}")
...
def prepare_jd_title(jd_description: str) -> str:
    """
    Prepare JD job title/description for embedding.
    Uses the job_description field as it contains the position/role.
    """
    if pd.isna(jd_description) or not str(jd_description).strip():
        return ""
    return str(jd_description).strip()


def prepare_cv_skills(skills: str) -> str:
    """
    Prepare CV skills text for embedding.
    """
...
def process_similarity(
    ...
    """
    Process JD and CV dataframes to compute JobBERT similarity scores.

    Args:
        jd_df: DataFrame with JobID and job_description columns
        cv_df: DataFrame with UserID and skills columns
        model: Optional pre-loaded model (loads if not provided)

    Returns:
        Tuple of (scores_df, similarity_matrix)
        scores_df has columns: JobID, UserID, jobbert_similarity_score
    """
    if model is None:
        model = load_jobbert_model()

    # Prepare texts
    logger.info("Preparing texts for embedding...")
    jd_titles = jd_df['job_description'].apply(prepare_jd_title).tolist()
    cv_skills = cv_df['skills'].apply(prepare_cv_skills).tolist()

    # Compute embeddings
    jd_embeddings = compute_embeddings(jd_titles, model)
        ...
        for cv_idx, user_id in enumerate(cv_df['UserID']):
            if jd_idx < len(similarity_matrix) and cv_idx < len(similarity_matrix[jd_idx]):
                scores.append({
                    'JobID': job_id,
                    'UserID': user_id,
                    'jobbert_similarity_score': float(similarity_matrix[jd_idx, cv_idx])
                })
...
def main():
    """
    Main entry point for JobBERT-v2 similarity computation.
    ...
    output_path = project_root / "preprocessed" / "jobbert_similarity_scores.csv"

    logger.info("=" * 60)
    logger.info("JobBERT-v2 Similarity Computation")
    logger.info("=" * 60)
    ...
    if len(scores_df) > 0:
        logger.info(f"Score range: [{scores_df['jobbert_similarity_score'].min():.4f}, {scores_df['jobbert_similarity_score'].max():.4f}]")
        logger.info(f"Mean score: {scores_df['jobbert_similarity_score'].mean():.4f}")
