...
Uses timpal0l/mdeberta-v3-base-squad2 (multilingual, mDeBERTa-v3-base fine-tuned on SQuAD2).

All questions are in Vietnamese because the project data (CVs and JDs) is in Vietnamese.
The model performs extractive QA across 94 languages, so answers will be spans extracted
directly from the Vietnamese context, ensuring Vietnamese output.
This model was chosen for its high download count (259k) among multilingual QA models and
compatibility with the standard Hugging Face QuestionAnswering interface.
"""
...
def get_model_and_tokenizer():
    ...
    if _model is None or _tokenizer is None:
        logger.info("Loading QA model: timpal0l/mdeberta-v3-base-squad2 (multilingual)")
...
def batch_extract(
    ...
    Uses the singleton model loaded via get_model_and_tokenizer.
    """
    results = []
